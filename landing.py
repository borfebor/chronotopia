...
def render_landing():
    """
    Render the pre-upload landing page.

    This MUST stay a function. It used to be module-level code invoked with
    `import landing`, but Python caches modules in sys.modules, so the body ran
    exactly once per process: on the second rerun with no file loaded, nothing
    rendered and the `st.stop()` never fired, leaving a blank page (reproducible
    by toggling "Generate example dataset" on and then off).
    """
